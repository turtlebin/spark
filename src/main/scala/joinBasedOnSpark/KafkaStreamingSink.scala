...
object KafkaStreamingSink {
  def main(args: Array[String]): Unit = {
    val sparkConf = new SparkConf().setAppName("KafkaStreamingSink").setMaster("spark://master:7077")
    val ssc = new StreamingContext(sparkConf, Minutes(3))
    ssc.checkpoint("hdfs://master:9000/checkpoint")
    //  ssc.checkpoint("/home/xhb/local/checkpoint")
//    val topicsSet = "joinMedian_1".split(",").toSet
//    val topicsSet2 = "joinMedian_2".split(",").toSet
    val topicsSet = "minJoin_1".split(",").toSet
    val topicsSet2 = "minJoin_2".split(",").toSet
    ...
    val kafkaProducer: Broadcast[KafkaSink[String, String]] = {
      val kafkaProducerConfig = {
        ...
        p.setProperty("bootstrap.servers", "master:9092,slave2:9092,slave3:9092")
    ...
    keyToList.foreachRDD(rdd => {
      rdd.foreachPartition(partition => {
        partition.foreach(x => {
          kafkaProducer
            .value 
            .send("update3", x._2.mkString("-"))
        })
      })
      kafkaProducer.unpersist()//该对象不能序列化，如果在worker内存不足需要把对象写到磁盘时是需要经过序列化阶段的，因此若不把这个东西unpersist，在内存不足要persist到磁盘的情况下，会出现unserializable错误
    })
    ssc.start()
